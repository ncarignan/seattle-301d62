...
// // the purpose of => ({}) is to return an obj literal
...
// you lose contextual this, so it can't become a constructor, so calling it with `new` breaks it
...
// if we need to reference the `Student` with `this` we can't use an arrow function
...
// we get an undefined because .scope does not return anything

Student.prototype.scopeArrow = () => console.log('scopearrowthis', this);
...
// this went one up in the scope, pointed at the `this` outside of joe.scopeArrow which is the Window, jow.scope pointed at joe;
